...
void MAXreg_wr(BYTE reg, BYTE val) {
	//psuedocode:
	XSpi_SetSlaveSelect(&SpiInstance, 1);//select MAX3421E
	BYTE wvals[2];
	wvals[0] = reg + 2;
	wvals[1] = val;
	int rcode = XSpi_Transfer(&SpiInstance, wvals, NULL, 2);//write reg + 2 via SPI
	//write val via SPI
	...
	XSpi_SetSlaveSelect(&SpiInstance, 0);//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	//psuedocode:
	XSpi_SetSlaveSelect(&SpiInstance, 1);//select MAX3421E (may not be necessary if you are using SPI peripheral)
	BYTE wvals[nbytes + 1];//write reg + 2 via SPI
	...
	int rcode = XSpi_Transfer(&SpiInstance, wvals, NULL, nbytes+1);//if return code != 0 print an error
	if(rcode != 0){
		xil_printf("error");
...
BYTE MAXreg_rd(BYTE reg) {
	//psuedocode:
	XSpi_SetSlaveSelect(&SpiInstance, 1);//select MAX3421E (
	BYTE wvals[2];//write reg via SPI
	wvals[0] = reg;//read val via SPI
	BYTE wvalr[2];
	int rcode = XSpi_Transfer(&SpiInstance, wvals, wvalr, 2);//read return code from SPI peripheral
	if(rcode != 0){
		xil_printf("error");
	}//if return code != 0 print an error
	XSpi_SetSlaveSelect(&SpiInstance, 0);
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	//psuedocode:
	XSpi_SetSlaveSelect(&SpiInstance, 1);//select MAX3421E (may not be necessary if you are using SPI peripheral)
	BYTE wvals[nbytes+1];//write reg via SPI
	BYTE wvalr[nbytes+1];//read return code from SPI peripheral
	wvals[0] = reg;
	int rcode = XSpi_Transfer(&SpiInstance, wvals, wvalr, nbytes+1);//if return code != 0 print an error
