...
WriteBuffer[0] = reg + 2;
WriteBuffer[1] = val;
returncode = XSpi_Transfer(&SpiInstance, WriteBuffer, NULL, 2);
if(returncode != 0)
{
xil_printf("Err");
}
...
returncode = XSpi_Transfer(&SpiInstance, WriteBuffer, NULL, nbytes+1);
if(returncode != 0)
{
xil_printf("Erros");
}
...
int returncode;
WriteBuffer[0] = reg;
returncode = XSpi_Transfer(&SpiInstance, WriteBuffer, ReadBuffer, 2);
//returncode = XSpi_Transfer(&SpiInstance, &ReadBuffer[0], &ReadBuffer[0], 1);
if(returncode != 0)
{
xil_printf("Mistake");
}
...
int returncode;
WriteBuffer[0] = reg;
//XSpi_Transfer(&SpiInstance, &WriteBuffer[0], NULL, 1);
returncode = XSpi_Transfer(&SpiInstance, WriteBuffer, ReadBuffer, nbytes+1);
if(returncode != 0)
{
xil_printf("Error");
}
//copy ReadBuffer to Data
XSpi_SetSlaveSelect(&SpiInstance, 0);
for(int j = 0; j < nbytes; j++)
{
data[j] = ReadBuffer[j+1];
}
return (data + nbytes);
}
